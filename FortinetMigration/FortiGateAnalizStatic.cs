...
{
    class FortiGateAnalizStatistic : VendorAnalizStatistic
    {
                                            ...
                                            List<CheckPoint_NAT_Rule> _cpNatRules)
        {
            ...
            foreach (var rule in _cpNatRules)
            {
                ...
                if (rule.Source != null && (rule.Source.Name == "any" | rule.Source.Name == "ANY")) { _rulesServicesutilizingServicesAnySourceCount++; }
                if (rule.Destination != null && (rule.Destination.Name == "any" | rule.Destination.Name == "ANY")) { _rulesServicesutilizingServicesAnyDestinationCount++; }
                if (rule.Service != null && (rule.Service.Name == "any" | rule.Service.Name == "ANY")) { _rulesServicesutilizingServicesAnyDestinationCount++; }
            }
            foreach (var package in _cpPackages)
            {
                ...
                foreach (var rule in package.ParentLayer.Rules)
                {
                    ...
                    if (rule.Source.Where(sr => sr.Name == "any" | sr.Name == "ANY").Count() != 0) { _rulesServicesutilizingServicesAnySourceCount++; }
                    if (rule.Destination.Where(dst => dst.Name == "any" | dst.Name == "ANY").Count() != 0) { _rulesServicesutilizingServicesAnyDestinationCount++; }
                    if (rule.Service.Where(srv => srv.Name == "any" | srv.Name == "ANY").Count() != 0) { _rulesServicesutilizingServicesAnyDestinationCount++; }
                ...
                foreach (var subPolicy in package.SubPolicies)
                {
                    ...
                    foreach (var subRule in subPolicy.Rules)
                    {
                        ...
                        if (subRule.Source.Where(sr => sr.Name == "any" | sr.Name == "ANY").Count() != 0) { _rulesServicesutilizingServicesAnySourceCount++; }
                        if (subRule.Destination.Where(dst => dst.Name == "any" | dst.Name == "ANY").Count() != 0) { _rulesServicesutilizingServicesAnyDestinationCount++; }
                        if (subRule.Service.Where(srv => srv.Name == "any" | srv.Name == "ANY").Count() != 0) { _rulesServicesutilizingServicesAnyDestinationCount++; }
